...
class Selector(ParentTask):
    ...
    Pour choisir un enfant, elle commence par le début de la liste des enfants et les essaye un par un jusqu'à ce qu'elle en trouve un qui passe le test check_condition.  
  it then updates that child until its finished.

    Si l'enfant finit avec échec, le Selecteur continue 
    à parcourir la liste en cherchant un autre enfant à update. S'il ne réussit pas, le Selecteur finit avec échec .
    Si l'enfant finit avec succès, le Sélecteur considère que sa tâche est fait et retourne succès."""
    ...
    def child_failed(self):
        """Dans le cas où l'enfant actuel finit avec un échec,
         on trouve un nouvel à update, ou si aucun enfant n'est trouvé, le parent task échoue ."""
